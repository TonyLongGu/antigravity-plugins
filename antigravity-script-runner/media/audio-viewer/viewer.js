...
(function () {
  ...
  window.addEventListener('error', (event) => {
    console.error('[AudioViewer UI Error]', event.error || event.message);
    if (vscode) {
      vscode.postMessage({
        type: 'showError',
        message: `音訊檢視器前端異常: ${event.message || '未知錯誤'}`
      });
    }
  });

  window.addEventListener('unhandledrejection', (event) => {
    console.error('[AudioViewer UI Unhandled Rejection]', event.reason);
    if (vscode) {
      vscode.postMessage({
        type: 'showError',
        message: `音訊檢視器未處理非同步錯誤: ${event.reason?.message || event.reason || '未知原因'}`
      });
    }
  });
