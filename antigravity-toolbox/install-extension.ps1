<#
.SYNOPSIS
    IDE 擴充套件一鍵安裝腳本 (智慧偵測 VS Code / Cursor / Antigravity 等相容 IDE + Junction 免編譯掛載 + extensions.json 註冊)
#>
...
$candidateTargets = @(
    ...
    [PSCustomObject]@{
        ...
        ExtensionsRoot = (Join-Path $env:USERPROFILE ".vscode-oss\extensions")
    },
    [PSCustomObject]@{
...
Write-Host "  安裝 IDE 原生擴充套件 (多 IDE 相容)     " -ForegroundColor Cyan
    ...
    if (Test-Path -LiteralPath $baseRoot) {
        ...
        foreach ($diskItem in $existingDiskItems) {
            ...
            if ($diname -like "$fullExtId*" -or $diname -like "$extName*" -or $diname -like "antigravity-toolkit.$extName*") {
                if ($diname.StartsWith("antigravity-toolkit.") -or $diname.StartsWith("antigravity-toolbox") -or $diname.StartsWith("antigravity-")) {
                    $shouldDelete = $true
                }
            }
...
if ($successCount -gt 0) {
    Write-Host "擴充套件安裝成功！請於目標 IDE（VS Code / Cursor / Antigravity）按 [Ctrl + Shift + P] -> 執行 [Developer: Reload Window] 生效。" -ForegroundColor Green
}
